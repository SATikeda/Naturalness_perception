function [hstPe, hstPf, hstCe, hstCf] = getFeatures( Xp, Xc, indices, comb, thsE, thsF )
...
% allocate memory
hstPe = zeros(n_bits^2, n_ptn, plane, n_data, 'uint32');
...
parfor f=1:n_data
    ...
    %calculate histogram
    hstPe(:,:,:,f) = calcHist3combBits(myRgb2yuv(samples), comb, thsE);
    f
end

%allocate memory
hstCe = zeros(n_bits^2, n_ptn, plane, n_data, 'uint32');
fprintf(1, 'calculating histograms...\n');
...
parfor f=1:n_data
    ...
    % calculate histogram
    hstCe(:,:,:,f) = calcHist3combBits(myRgb2yuv(samples), comb, thsE);
    f
end

% allocate memory
hstPf = zeros(n_bits^2, n_ptn, plane, n_data, 'uint32');
fprintf(1, 'calculating histograms...\n');
...
parfor f=1:n_data
    ...
    % calculate histogram
    hstPf(:,:,:,f) = calcHist3combBits(myRgb2yuv(samples), comb, thsF);
    f
end

% allocate memory
hstCf = zeros(n_bits^2, n_ptn, plane, n_data, 'uint32');
fprintf(1, 'calculating histograms...\n');
...
parfor f=1:n_data
    ...
    % calculate histogram
    hstCf(:,:,:,f) = calcHist3combBits(myRgb2yuv(samples), comb, thsF);
    f
end
